// src/components/portfolio/investment/InvestmentDocumentsTab.tsx
...
interface InvestmentDocumentsTabProps {
  portfolioId: string;
}

export function InvestmentDocumentsTab({ portfolioId }: InvestmentDocumentsTabProps) {
  ...
  const handleDelete = async (documentId: string) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce document ?')) {
      try {
        ...
        if (success) {
          setDocuments(prev => prev.filter((doc): doc is PortfolioDocument => doc.id !== documentId));
